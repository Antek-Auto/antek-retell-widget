...
export default function Dashboard() {
  ...
  const { user, profile, loading, signOut, userRole, isSuperAdmin, isAdmin: isAdminUser } = useAuth();
  ...
  const getWidgetLimit = (): number => {
    if (isSuperAdmin) return Infinity;
    if (isAdminUser) return 500;
    return 100;
  };

  const widgetLimit = getWidgetLimit();
  ...
  const createWidget = async () => {
    ...
    if (!canCreateWidget) {
      const limitText = widgetLimit === Infinity ? "unlimited" : widgetLimit;
      toast.error(`You've reached the maximum limit of ${limitText} widgets.`);
      return;
    }
